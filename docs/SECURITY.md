...
The 2.1.2 release adds no third-party hosts, no new media or connect sources, no new external scripts, and no new file types. The new playing banner element renders via `textContent` and `replaceChildren`. The two new clear buttons share their click handlers with the existing combobox X icons and add no new code paths into the audio resolver. `web-ext lint --self-hosted` improved on the Chrome build: 0 errors and 10 warnings, down from the 2.1.1 baseline of 2 errors and 9 warnings. Adding `browser_specific_settings.gecko.id` and a `background.scripts` fallback to the Chrome manifest cleared `BACKGROUND_SERVICE_WORKER_NOFALLBACK` and `ADDON_ID_REQUIRED`; Chrome ignores both fields. Firefox lint remains at 0 errors, 4 warnings.
...
A code-hygiene pass deleted the unused `shared/hadith.js` module and dropped three orphan exports (`isFirefox`, `parseReciterKey`, `isCoverageFresh`) that no caller imported. Firefox's French hadith pipeline was rewritten to mirror Chrome's: JSDelivr-pinned editions on the existing CSP allowlist, a 30-entry storage cache, and consistent fallback strings across both builds.
