...
// the background-page audio, clears lastKnownAudioState so the popup
// forgets which track was alive, and refreshes the play-button surface for
// the current (different) selection.
async function discardPlayingAudio() {
  try {
    await browser.runtime.sendMessage({ action: 'pauseAudio' });
  } catch (err) {
